            ...
            while let Poll::Ready(ev) = kademlia.poll(cx, params) {
                match ev {
                    NetworkBehaviourAction::GenerateEvent(ev) => match ev {
                        ...
                        QueryResult { result: QueryResult::GetClosestPeers(res), .. } => {
                            match res {
                        ...
                        QueryResult { result: QueryResult::GetRecord(res), stats, .. } => {
                            let ev = match res {
                                Ok(ok) => {
                        ...
                        QueryResult { result: QueryResult::PutRecord(res), stats, .. } => {
                            let ev = match res {
                        ...
                        QueryResult { result: QueryResult::RepublishRecord(res), .. } => {
                            match res {
